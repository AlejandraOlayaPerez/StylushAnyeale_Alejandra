<!DOCTYPE html>
<html lang="en">

<head>
    <?php require_once 'linkhead.php'; ?>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="/anyeale_proyecto/stylushanyeale_alejandra/assets/css/nosotros.css" type="text/css">
    <link rel="shortcut icon" href="/anyeale_proyecto/stylushanyeale_alejandra/image/logo.png" type="image/x-icon">
    <title>Stylush Anyeale</title>
</head>

<body>
    <section class="candidates-profile-bg">
        <div class="bg-overlay"></div>
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-md-6">
                    <div class="candidates-profile-details text-center">
                        <img src="../image/PNG_LOGO.png" class="img-fluid d-block mx-auto rounded-circle img-thumbnail mb-4">
                        <h4 class="text-white candidates-profile-name mb-2">Stylush Anyeale</h4>
                        <ul class="candidates-profile-social-icons list-inline mb-0">
                            <li class="list-inline-item"><a href="https://www.facebook.com/Stylush-Anyeale-219067613464253/" class=""><i class="mdi mdi-facebook"></i></a></li>
                            <li class="list-inline-item"><a href="https://www.instagram.com/__anyeale/?hl=es" class=""><i class="mdi mdi-instagram"></i></a></li>
                            <li class="list-inline-item"><a href="https://www.youtube.com/channel/UCAIAHNzp71toK21oTYZtN1Q" class=""><i class="mdi mdi-youtube"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
    </section>

    <section class="section">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <h3 class="text-dark" style="font-family: 'Times New Roman', Times, serif;">Sobre Nosotros</h3>
                </div>
            </div>

            <div class="row">
                <div class="col-lg-12">
                    <div class="job-detail mt-2 p-4">
                        <div class="job-detail-desc">
                            <p class="text-muted f-14 mb-3" style="font-family: 'Times New Roman', Times, serif;">Stylush Anyeale quiere ofrecer a sus clientes una amplia oferta de estrategias que beneficien a los clientes como a empleados. De esta manera general una mejor comercialización de los servicios que se ofrecen en las diferentes salas de belleza del Guaviare. </p>

                            <p class="text-muted f-14 mb-0" style="font-family: 'Times New Roman', Times, serif;">Al crear nuevas estrategias permitirá un abarque de más personas que podrían estar interesadas, de igual manera solo se busca ayudar y general un mejor desempeño en las prestaciones de servicios, ofreciendo a los clientes más fácil acceso a los servicios o información correspondiente, a la igual manera también beneficia a los establecimientos, porque permitirá tener la información mas organizada y con mejor calidad, sin llegar a estar distorsionada.</p>
                        </div>
                        
                    </div>
                </div>
            </div>
        </div>
    </section>
</body>

</html>